...
async function buscarFactura(req, res) {
    try {
        ...
        // Intentar primero con SinNumero = true, luego false
        const estadosNumero = [true, false];
        let responseData = null;

        for (const estado of estadosNumero) {
            const url = `https://balance.saludplus.co/facturasAdministar/BuscarListadofacturasDatos?fechaInicial=01/01/2020&fechaFinal=01/01/2029&idEntidad=0&idContrato=0&SinNumero=${estado}&duplicadas=False&idCuentaCobro=0&estadoFacturacionElectronica=0`;
            ...
            if (response.data.aaData && response.data.aaData.length > 0) {
                responseData = response.data.aaData;
                break; // salió bien, no necesitamos seguir buscando
            }
        }
        ...
        // Mapear facturas
        const facturas = responseData.map(f => ({
            id: f[0],
            numeroCompleto: f[2],
