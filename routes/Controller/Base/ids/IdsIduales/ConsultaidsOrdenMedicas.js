// Archivo: ConsultaidsOrdenMedicas.js (función base)
const axios = require('axios');

const ConsultaIdOrdenMedicas = async (documento) => {
    try {
        if (!documento) {
        ...
        const cookies = [
            ...
            '_ga_581YHK4S33=GS2.1.s1774201635`$o11`$g1`$t1774203186`$j60`$l0`$h0',
            '_clsk=hz2ilc%5E1774203207380%5E12%5E1%5Eb.clarity.ms%2Fcollect'
        ];
        ...
        const headers = {
            ...
            'referer': 'https://balance.saludplus.co/instituciones/?origen=1&theme=false&time=1774203186783',
        ...
        if (response.data && response.data.aaData) {
            const registroEncontrado = response.data.aaData.find(orden => 
                orden[1] === documento.toString()
            );
            
            if (registroEncontrado) {
                const idOrden = registroEncontrado[0];
                return '20' + idOrden; // Devuelve el número de admisión con 20 al inicio
            } else {
                throw new Error('Documento no encontrado en órdenes médicas');
            }
        } else {
            throw new Error('No se encontraron resultados en órdenes médicas');
        }

    } catch (error) {
        console.error('Error en ConsultaIdOrdenMedicas:', error.message);
        throw error;
    }
};
